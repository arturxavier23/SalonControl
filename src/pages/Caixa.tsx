      ...
      <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="rounded-xl border border-white/10 bg-white/5 px-5 py-3 backdrop-blur-sm">
            <p className="text-sm text-zinc-400">Caixa da data</p>
          ...
          <div className="rounded-xl border border-white/10 bg-white/5 px-5 py-3 backdrop-blur-sm">
            ...
            <input
              type="date"
              className="rounded-lg border border-white/10 bg-zinc-950/40 px-3 py-2 outline-none focus:border-violet-500 focus:ring-2 focus:ring-violet-500/20"
              value={dataFiltro}
          ...
          {usuarioAdministrador && (
            <div className="rounded-xl border border-white/10 bg-white/5 px-5 py-3 backdrop-blur-sm">
              <p className="mb-1 text-sm text-zinc-400">Fechamento</p>
              {caixaFechado ? (
                <button
                  type="button"
                  onClick={reabrirCaixa}
                  disabled={fechando}
                  className="rounded-lg border border-white/15 px-3 py-2 text-sm text-zinc-200 hover:bg-white/5 hover:text-white disabled:opacity-50"
                >
                  Reabrir caixa
                </button>
              ) : (
                <button
                  ...
                  className="rounded-lg bg-gradient-to-r from-emerald-600 to-teal-600 px-3 py-2 text-sm font-semibold text-white shadow-lg shadow-emerald-950/40 hover:from-emerald-500 hover:to-teal-500 disabled:cursor-not-allowed disabled:opacity-50"
                >
                  Fechar caixa
                </button>
              )}
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        >
          ...
          <div>
            ...
            <select
              className="w-full rounded-lg border border-white/10 bg-zinc-950/40 px-3 py-2 outline-none focus:border-violet-500 focus:ring-2 focus:ring-violet-500/20 disabled:cursor-not-allowed"
              value={agendamentoId}
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 p-5 shadow-xl shadow-black/20 backdrop-blur-sm lg:col-span-2">
          ...
          {carregando ? (
            ...
            <p className="text-zinc-400">Nenhuma venda registrada nesta data.</p>
          ) : (
            <div className="space-y-3">
              {vendas.map((venda) => (
                <div
                  key={venda.id}
                  className="flex items-start justify-between gap-4 rounded-lg border border-white/10 bg-zinc-950/40 p-4"
                >
                  <div>
